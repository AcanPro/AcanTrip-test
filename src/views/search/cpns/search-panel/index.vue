<template>
	<div class="search-panel">
		<van-search v-model="value" show-action shape="round" placeholder="搜索广州的景点、地标、房源" @search="onSearch"
			@cancel="onCancel" />
		<div v-if="searchsuggest.length" class="search-result-panel">
			<template v-for="(item, index) in searchsuggest" :key="index">
				<search-result-item :key-word="value" :name="item.name" :type-name="item.itemTypeName"
					:description="item.description" :unitPrice="item.unitPrice" @item-click="handleResultItemClick" />
			</template>
		</div>
		<div v-else class="search-category-panel">
			<search-category title="搜索历史" :itemDatas="searchStore.getSearchHistorys" itemType="pink"
			...
			<template v-for="(item, index) in searchPanelDatas" :key="index">
				<search-category v-if="item.suggests" :title="item.name" :itemDatas="item.suggests"
					:itemType="index === 0 ? 'pink' : 'gray'" @tag-click="handleTagClick($event, item)"></search-category>
			</template>
		</div>
	</div>
</template>
...
const handleTagClick = (subItem, item) => {
	const { name } = storeToRefs(item)
	emit("tagClick", { typeName: name, keyWord: subItem.name });
};
